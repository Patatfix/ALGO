...
def lire_fichier(nom_fichier): #prend un fichier.txt et renvoie une liste de mots (en enlevant les retour a la ligne et les potentiels espaces en début de texte)
    with open(nom_fichier, "r", encoding="utf-8") as f:
        texte = f.read()
        try:
            if texte[-1] == '\n': #enleve les sauts de lignes
                texte = texte[:-1]
            i=0
            while texte[i]==' ': #enleves les espaces en debut de texte
                i+=1
            texte = texte[i:]
            texte = texte.replace("\n", " ") #remplace les retours a la lignes par des espaces pour prendre tout types de texte
            liste_mots = texte.split(" ") #fait une liste de mots (séparés par des ' ')
        except : 
...
recherche_p1 = lire_fichier("recherche_p1.txt")
exemple1 = lire_fichier("exemple1.txt")
recherche_complet = lire_fichier("recherche_complet.txt")
...
test_texte(recherche_p1, 60, exhaustif_backtracking, 150)
test_texte(recherche_p1, 60, glouton, 150)
test_texte(recherche_p1, 60, dico_to_texte, 150)
